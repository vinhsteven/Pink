...
public class DbClassShoppingCart extends JdbcAccountsDBTemplate {
	...
	// param is cartId
	private String getSavedItemsQuery = "SELECT * FROM shopcartitem sci, productsdb.product p WHERE shopcartid = ? and sci.productid = p.productid";
	// param is cartId
	...
	private List<CartItem> getSavedCartItems(Integer cartId) throws DatabaseException {
		return jdbcTemplate.query(getSavedItemsQuery, new RowMapper<CartItem>(){
			...
			public CartItem mapRow(ResultSet rs, int rowNum) throws SQLException {
					...
					try {
						CartItemImpl ci = 
						new CartItemImpl(rs.getInt("shopcartid"), rs.getInt("productid"), rs.getInt("cartitemid"),
									makeString(rs.getInt("quantity")), makeString(rs.getDouble("totalprice")), true);
						ci.setCatalogid(rs.getInt("catalogid"));
