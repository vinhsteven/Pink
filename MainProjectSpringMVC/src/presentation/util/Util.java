...
private final static String REL_RULES_PATH = "business/rulefiles";
	...
	public static List<CartItem> cartItemPresToCartItemList(List<CartItemPres> list) {
		...
		return list.stream()
				...
				.map(cartdata -> ShoppingCartSubsystemFacade.createCartItem(cartdata.getProductId(),cartdata.getItemName(), 
	...
	public static List<CartItemData> cartItemsToCartItemData(List<CartItem> list) {
		if(list == null || (list.size() == 1 && list.get(0).getCartid() == null))
			return new ArrayList<CartItemData>();
